...
void cmdNick(Server& server, Client::ClientInfo& sender, int socketFd, const Message& msg)
{
	...
	if (sender.isRegistered)
	{
		...
		{
			std::vector<int>& users = channelList[i].getUser();
			//member is in channel
			bool isMember = false;
			...
			// if in channel add all members of channel in the recipients list
			if (isMember)
			{
				for (size_t j = 0; j < users.size(); ++j)
				{
					recipients.insert(users[j]); // std::set suppr dupes
				}
			}
		}
		//print
